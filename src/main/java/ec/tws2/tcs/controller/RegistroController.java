...
public class RegistroController implements Serializable {
    ...
    private final String[] nombresCiudades = {"Alamor",
        "Alausí",
    ...
    public void registrarUsuario() {
        try {
            uploadLogo();
            uploadFirma();
            if (cliente.getIdentification().isEmpty()) {
                mensaje("Identificación no Ingresada");
            } else if (cliente.getNombres().isEmpty()) {
                mensaje("Nombres no Ingresados");
            } else if (cliente.getApellidos().isEmpty()) {
                mensaje("Apellidos no Ingresados");
            } else if (cliente.getCiudad().isEmpty()) {
                mensaje("Ciudad no Seleccionada");
            } else if (cliente.getTelefono().isEmpty()) {
                mensaje("Teléfono no Ingresado");
            } else if (cliente.getClaveFirma().isEmpty()) {
                mensaje("Clave Firma no Ingresada");
            } else if (cliente.getArchivoFirma().isEmpty()) {
                mensaje("Archivo Firma no Ingresado");
            } else {
                ...
                String respuesta = apiCall.guardarUsuario(cliente.getArchivoFirma(), cliente.getIdentification(), cliente.getClaveFirma(), cliente.getNombres(), cliente.getApellidos(), "", cliente.getCiudad(), cliente.getTelefono(), 1, email, username, cliente.getLogo());
                if (respuesta.equals("1")) {
                    mensaje("Se produjo un error revise los Datos Ingresados");
                } else {
                    logout();
                }
            }
    ...
    public void uploadFirma() {
        if (firmaFile != null) {
            try (InputStream inputStream = firmaFile.getInputStream()) {
                byte[] firmaBytes = new byte[(int) firmaFile.getSize()];
                inputStream.read(firmaBytes);
                String base64Firma = Base64.getEncoder().encodeToString(firmaBytes);
                cliente.setArchivoFirma(base64Firma);
            } catch (Exception e) {
            }
        }
    }
    ...
    public void uploadLogo() {
        if (logoFile != null) {
            try (InputStream inputStream = logoFile.getInputStream()) {
                byte[] imageBytes = new byte[(int) logoFile.getSize()];
                inputStream.read(imageBytes);
                String base64Image = Base64.getEncoder().encodeToString(imageBytes);
                cliente.setLogo(base64Image);
            } catch (Exception e) {
            }
        }
    }
